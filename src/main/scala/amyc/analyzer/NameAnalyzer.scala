...
object NameAnalyzer extends Pipeline[N.Program, (S.Program, SymbolTable)] {
  def run(ctx: Context)(p: N.Program): (S.Program, SymbolTable) = {
    ...
    //(owner, name) -> fd
    var nameToClassOrFunDef = Map.empty[(String, String), N.ClassOrFunDef]
    ...
    for {
      ...
      val argTypes = fields map (param => transformType(param.tt, m.name))
      val retType = table.getType(m.name, parent).getOrElse(fatal(s"Parent class $parent not found", cc))
      nameToClassOrFunDef += ((m.name, name) -> cc)
    ...
    def transformFunDef(fd: N.FunDef, module: String): S.FunDef = {
      ...
      val newParams = params zip sig.argTypes map { case (pd@N.ParamDef(name, tt, value), tpe) =>
    ...
    def transformExpr(expr: N.Expr)
      ...
      val res = expr match {
        ...
        case N.Call(qname, args) =>
          var owner = qname.module.getOrElse(module)
          val name  = qname.name
          var entry = table
                      .getConstructor(owner, name)
                      .orElse(table.getFunction(owner, name))
          if(!entry.isDefined) {
            //println("(owner, name) = " + table.getModuleFromName(name))
            table.getModuleFromName(name) match
          ...
          entry match {
            ...
            case Some((sym, sig)) =>
              val classOrFunDef = nameToClassOrFunDef((owner, name))

              classOrFunDef match {
                case funDef@N.FunDef(n, p, r, b) => 

                  val defaultParameters = for (par, i) <- p.zipWithIndex if(par.value.isDefined) yield (par.value, i)

                  val nameToIndex = p.zipWithIndex.map((par, i) => (par.name, i)).toMap

                  //Prints used to visualize the different variables, parameters and arguments

                  //println("Args = " + args.zipWithIndex)
                  //println("Params = " + p)
                  //println("Default Params = " + defaultParameters)

                  //println("Name to index = " + nameToIndex)

                  var reorderedArgs: Array[Option[N.Expr]] = Array.fill(p.size) { None }

                  //println("reorderedArgs initial = " + reorderedArgs.toList)

                  for arg <- args.zipWithIndex do {
                    arg match
                      case ((None, v), i) => if reorderedArgs(i).isEmpty then reorderedArgs(i) = Some(v)
                                             else fatal("Error : multiple values for the same argument")
                      case ((Some(n), v), i) => if reorderedArgs(nameToIndex.getOrElse(n, fatal("Error : no such name " + n + " for arguments"))).isEmpty then reorderedArgs(nameToIndex(n)) = Some(v)
                                             else fatal("Error : multiple values for the same argument")
                  }

                  //println("reorderedArgs mid = " + reorderedArgs.toList)

                  for (defaultValue, i) <- defaultParameters do {
                    if reorderedArgs(i).isEmpty then reorderedArgs(i) = defaultValue
                  }

                  //println("reorderedArgs end = " + reorderedArgs.toList)

                  S.Call(sym, reorderedArgs.toList.map(v => transformExpr(v.getOrElse(fatal("Error : not enough arguments given")))))

                case caseClassDef@N.CaseClassDef(n, f, p) =>

                  val defaultParameters = for (par, i) <- f.zipWithIndex if(par.value.isDefined) yield (par.value, i)

                  val nameToIndex = f.zipWithIndex.map((par, i) => (par.name, i)).toMap

                  var reorderedArgs: Array[Option[N.Expr]] = Array.fill(f.size) { None }
