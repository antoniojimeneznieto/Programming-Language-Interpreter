  ...
  def apply(t: Tree)(implicit printUniqueIDs: Boolean = false): String = {
    ...
    def rec(t: Tree, parens: Boolean = true): Document = t match {
      ...
      case ParamDef(name, tpe, value) =>
        if(value.isDefined) then name <:> ": " <:> rec(tpe) <:> "= " <:> rec(value.get)
        else                     name <:> ": " <:> rec(tpe)
      ...
      case Call(name, args) =>
        ...
        "(" <:> 
        Lined(
          args map((name, expr) => name match
            case None => rec(expr)
            case Some(name) => name <:> "= " <:> rec(expr))
          , ", ") <:> 
        ")"
...
trait SymbolicPrinter extends Printer {
  ...
  def apply(t: Tree)(implicit printUniqueIDs: Boolean = false): String = {
    ...
    def rec(t: Tree, parens: Boolean = true): Document = t match {
      ...
      case FunDef(name, params, retType, body) =>
        Stacked(
          "fn " <:> printName(name) <:> "(" <:> Lined(params map (rec(_)), ", ") <:> "): " <:> rec(retType) <:> " = {",
          Indented(rec(body, false)),
          "}"
        )
      ...
      case Neg(e) =>
        "-(" <:> rec(e) <:> ")"
      case Call(name, args) =>
        printName(name) <:> "(" <:> Lined(args map (rec(_)), ", ") <:> ")"
      case Sequence(lhs, rhs) =>
        val main = Stacked(
          rec(lhs, false) <:> ";",
          rec(rhs, false),
      ...
      case TypeTree(tp) =>
        tp match {
          ...
          case StringType => "String"
          case UnitType => "Unit"
          case ClassType(name) => printName(name)
        }
