...
object Lexer extends Pipeline[List[File], Iterator[Token]]
  ...
  val lexer = Lexer(
    ...
    // Alpha ::= [a − z] | [A − Z]
    // Digit ::= [0 − 9]
    many1(elem(_.isLetter)) ~ many(elem(_.isLetterOrDigit) | elem('_'))
    ...
    many1(elem(_.isDigit))
      |> { (cs, range) =>
        try { 
          ...
          case e: Exception => ErrorToken(cs.mkString).setPos(range._1)
        } 
      },
        
    // String literal,
    word("\"") ~ many(elem(c => c != '\n' && c != '\"')) ~ word("\"")
      |> { (cs, range) => StringLitToken(cs.mkString.filter(c => c != '\"')).setPos(range._1) },
    ...
    word("/*") ~ many(elem(_ != '*') | (many1(elem('*')) ~ elem(c => c != '/' && c != '*'))) ~ many(elem('*'))
      |> { (cs, range) => ErrorToken(cs.mkString).setPos(range._1) },

    // Correct Multiline comment
    word("/*") ~ many(elem(_ != '*') | (many1(elem('*')) ~ elem(c => c != '/' && c != '*'))) ~ many1(elem('*')) ~ elem('/')
